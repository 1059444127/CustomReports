...
namespace CustomReports
{
    ...
    /// 统计项目合计数
    /// </summary>
    [Report("LCT工作量统计", "广中中山一")]
    public partial class ZsfyLctReport : XtraUserControl, IReport
    {
        public ZsfyLctReport()
        {
            InitializeComponent();
        ...
        public void Query()
        {
            ...
            try
            {
                ...
                sb.Append("sum(case when (f_tbszd like '%低级别鳞状上皮内病变%') then 1 else 0 end ) as LSIL,");
                sb.Append("sum(case when (f_tbszd like '%高级别鳞状上皮内病变%') then 1 else 0 end ) as HSIL,");
